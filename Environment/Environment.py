...
class SQLI_Environment:
    game_to_token = {
        Game_Type.BEHAVIOR_CHANGING: [Token.Category.BEHAVIOR_CHANGING],
        Game_Type.SYNTAX_FIXING: [
            Token.Category.BASIC_BLOCK,
            Token.Category.SYNTAX_FIXING,
        ],
        Game_Type.SANITIZATION_ESCAPING: [Token.Category.SANATISATION_ESCAPING],
        Game_Type.DONE: [],
    }
    ...
    def __init__(
        ...
        self.__reward_score = {
            ...
            Game_Type.SYNTAX_FIXING: {"win": 0, "loss": -1},
            Game_Type.SANITIZATION_ESCAPING: {"win": 0, "loss": -1},
            Game_Type.DONE: {"win": 0, "loss": 0},
        }
        pass
    ...
    def setup_inputs(self, url):
        ...
        if len(possible_inputs) == 0:
            ...
            sys.exit()
        filtered_inputs, sql_statements = self.__sql_filter.test_for_sql_statement(
            possible_inputs
        )
    ...
    def reset(self, change_input=False):
        # init for the game
        self.__escapes = 0
        if (change_input and not self.__input_storage.is_empty()) or (
            self.current_input_entry is None and not self.__input_storage.is_empty()
        ):
    ...
    def step(self, action):
        ...
        new_statement = self.__sql_filter.get_new_sql_statement(
        ...
        if (
            not self.__current_error
            and self.__current_game is not Game_Type.SANITIZATION_ESCAPING
            and self.current_input_entry.is_behavior_changed()
        ):
            self.__last_game = self.__current_game
        ...
        if self.__current_game == Game_Type.BEHAVIOR_CHANGING:
            if self.__last_game == Game_Type.SYNTAX_FIXING:
                ...
                if self.current_input_entry.is_identifier_in_context():
                    ...
                    flat_index_length = len(
                        self.current_input_entry.payload.base_token.flat_idx_tokens_list()
                    )
                    in_context_reward = self.__reward_score[self.__current_game][
                        "in_context"
                    ] * len(self.current_input_entry.in_context_elements())
                    out_context_reward = (
                        self.__reward_score[self.__current_game]["out_context"]
                        * (1 - len(self.current_input_entry.in_context_elements()))
                        / flat_index_length
                    )
                    ...
                    change_behviour = (
                        self.__reward_score[self.__current_game]["change_behavior"]
                        if self.current_input_entry.is_context_escaped()
                        else self.__reward_score[self.__current_game][
                            "unchanged_behavior"
                        ]
                    )
                    return -1
    ...
    def available_actions(self):
        ...
        out_of_context_starting_index = (
            self.current_input_entry.get_starting_out_of_context_payload_index()
        )

        if self.is_end_to_end:
            if out_of_context_starting_index:
                self.current_input_entry.payload.escaped_context = (
                    out_of_context_starting_index
                )
                action_game_map = {}
                for current_game in Game_Type.__iter__():
                    action_game_map[current_game] = (
                        self.current_input_entry.payload.available_actions(
                            SQLI_Environment.game_to_token[current_game],
                            out_of_context_starting_index,
                        )
                    )
            ...
            if out_of_context_starting_index:
                return self.current_input_entry.payload.available_actions(
                    SQLI_Environment.game_to_token[self.__current_game],
                    out_of_context_starting_index,
                )
            else:
                return self.current_input_entry.payload.available_actions(
