...
class Crawler:
    def __init__(
        ...
        if (
            login_module["module_path"] is not None
            and login_module["function"] is not None
        ):
            self.login_function = import_attr(
                login_module["module_path"], login_module["function"]
            )
        else:
    ...
    def crawl_possible_inputs(self, url):
            ...
            if int(level) <= int(self.max_depth_level):
                try:
                    ...
                    for current_link in copy_url_queue:
                        current_forms, current_dynamic_inputs = self.readlinks(
                            current_link
                        )
                        self.forms.extend(current_forms)
                        self.dynamic_inputs.extend(current_dynamic_inputs)
                ...
                if len(self.urls_queue) > 0 and self.verbose > 0:
                    print(
                        "[-] stoping crawling due to max depth reached and urls queue not empty"
                    )
                break
        ...
        inputs = [
            Input(
                current_possible_input,
                self.input_tokinizer.get_next_token(),
                self.login_function,
            )
            for current_possible_input in multivariables
        ]
        return inputs
    ...
    def readlinks(self, url):
        ...
        for form in soup.find_all("form"):
            ...
            if details is not None:
                ...
                if any(
                    [
                        current_action == current_details["action"]
                        and current_inputs
                        == Crawler.get_input_names(current_details["inputs"])
                        for current_details in forms
                    ]
                ) or any(
                    [
                        current_action == current_details["action"]
                        and current_inputs
                        == Crawler.get_input_names(current_details["inputs"])
                        for current_details in self.forms
                    ]
                ):
                    pass
                else:
        ...
        for link in links_to_be_explored:
            ...
            if link.get("href") is not None:
                        ...
                        if "?" in link.get("href"):
                            ...
                            if any(
                                [
                                    current_action == current_details["action"]
                                    and current_inputs
                                    == Crawler.get_input_names(
                                        current_details["inputs"]
                                    )
                                    for current_details in dynamic_urls
                                ]
                            ) or any(
                                [
                                    current_action == current_details["action"]
                                    ...
                                    for current_details in self.dynamic_inputs
                                ]
                            ):
                                pass
                            else:
                                ...
                                dynamic_url_details = self.get_dynamic_link_details(
                                    link.get("href")
                                )
                            ...
                            if all(
                                [
                                    proposed_url != current_url
                                    for current_url in self.urls_queue
                                ]
                            ):
                                self.urls_queue.append(link.get("href"))
                # IF URL IS DYNAMIC
                elif "?" in link.get("href"):
                    details = self.get_dynamic_link_details(
                        url + "/" + link.get("href")
                    )
                    # compare the current details with the past data
                    # compare by action url
                ...
                # ELSE NORMAL LINK FOUND
                    ...
                    if all(
                        [proposed_url != current_url for current_url in self.urls_queue]
                    ):
                        self.urls_queue.append(
                            re.sub(r"/#.*", "", url + "/" + link.get("href"))
                        )
    ...
    def get_form_details(self, form, url):
        ...
        if form.attrs.get("action") is None or str(form.attrs.get("action")) == "":
            ...
            if str(url_parsed.scheme) in action:
                ...
                action = (
                    str(url_parsed.scheme)
                    + "://"
                    + str(url_parsed.netloc)
                    + "/"
                    + str(action)
                )
    ...
    def get_dynamic_link_details(self, url: str):
        ...
        variables = [
            {"name": current_var.split("=")[0], "value": current_var.split("=")[1]}
            ...
            for current_input in range(len(current_form["inputs"])):
                ...
                for new_current_input in range(len(current_new_form["inputs"])):
                    if current_input == new_current_input:
                        current_new_form["inputs"][new_current_input]["input_use"] = (
                            "injection_point"
                        )
                    else:
                        current_new_form["inputs"][new_current_input]["input_use"] = (
