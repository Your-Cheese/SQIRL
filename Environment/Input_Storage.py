...
class Input_storage:
    ...
    def add_new_inputs(self, filtered_inputs, sql_statments):
        ...
        for current_index, current_input_current_sql in enumerate(
            list(zip(filtered_inputs, sql_statments))
        ):
            current_input, current_sql = (
                current_input_current_sql[0],
                current_input_current_sql[1],
            )
            inputs_found.write(
                f"%%%%%%%%%%%%%%%%%%%%%%%%%%%%{current_index}%%%%%%%%%%%%%%%%%%%%%%%%%%%%\n"
            )
        ...
        new_entries = [
            Entry(current_input, current_sql_stmt, Payload(current_input.token))
            for current_input, current_sql_stmt in zip(filtered_inputs, sql_statments)
        ]
...
class Entry:
    ...
    def is_payload_token_sanatized(
        self, paylaod_flat_token_idx: int
    ) -> Union[int, None]:
        """
    ...
    def is_escaped_context(self):
        ...
        for current_token_idx in range(len(tokens)):
            if sql_statement.is_affective_behviour_changing(
                self.payload, identifier_token_index, current_token_idx
            ):
                return True
    ...
    def is_behavior_changed(self):
        ...
        for current_token_idx in range(len(tokens)):
            if (
                sql_statement.is_affective_behviour_changing(
                    self.payload, identifier_token_index, current_token_idx
                )
                and tokens[current_token_idx].category()
                == Token.Category.BEHAVIOR_CHANGING
            ) or sql_statement.is_keyword_after_payload_commented(
                self.payload, identifier_token_index
            ):
                return True
