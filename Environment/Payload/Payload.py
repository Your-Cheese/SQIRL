...
from Environment.Tokens_Actions.Sanitization_Escaping.Capatlize_Action import (
    Capatilize_Action,
)
from Environment.Tokens_Actions.Sanitization_Escaping.Represerntation_Action import (
    Representation_Action,
)
from Environment.Tokens_Actions.Syntax_Fixing.StringGeneration_Action import (
    StringGeneration_Action,
)
...
class Payload:
    ...
    def apply_action(self, action_payload: list):
        ...
        if action == 0:  # add comma
            ...
            self.base_token.replace_range_tokens_base_idx(
                pos_start, pos_end, comment_range_token
            )
            ...
            quote_type = random.randint(
                0, len(Quote_Token.quote_type_mapping.keys()) - 1
            )
            token = CONCAT_Token(
                type,
                [
                    String_Token(string_1, quote_type),
                    String_Token(string_2, quote_type),
                ],
            )
            # remove quote before and after
            self.base_token.remove_token_flat_idx(pos - 1)
            ...
            assert self.base_token.is_valid_flat_idx(pos)
            slash = Slash_Token()
            self.base_token.replace_flat_idx_token_with_tokens(
                pos, [slash, self.base_token.get_token_flat_idx(pos)]
            )
        else:  # invalid action
            assert (False, "invalid action")

    def available_actions(
        self, action_category: list, out_of_contex_starting_index=None
    ):
        ...
        if Token.Category.SYNTAX_FIXING in action_category:
            ...
            for current_pos in range(1, no_tokens + 1):
                if current_pos < no_tokens and isinstance(
                    self.base_token.token_list[current_pos], FullComment_Token
                ):
                    break
                else:
                    actions.append(
                        {"action": 1, "range": [current_pos, current_pos], "type": 0}
                    )
            ...
            for current_pos in range(1, no_tokens + 1):
                if current_pos < no_tokens and isinstance(
                    ...
                    actions.append(
                        {"action": 3, "range": [current_pos, current_pos], "type": 1}
                    )
                    break
                actions.append(
                    {"action": 3, "range": [current_pos, current_pos], "type": 1}
            ...
            for current_pos in range(1, no_tokens + 1):
                if current_pos < no_tokens and isinstance(
                    ...
                    actions.append(
                        {"action": 4, "range": [current_pos, current_pos], "type": 0}
                    )
                    break
                actions.append(
                    {"action": 4, "range": [current_pos, current_pos], "type": 0}
            ...
            for current_pos in range(1, no_tokens + 1):
                if current_pos < no_tokens and isinstance(
                    ...
                    actions.append(
                        {"action": 4, "range": [current_pos, current_pos], "type": 1}
                    )
                    break
                actions.append(
                    {"action": 4, "range": [current_pos, current_pos], "type": 1}
            ...
            for current_pos in range(1, no_tokens + 1):
                if current_pos < no_tokens and isinstance(
                    ...
                    actions.append(
                        {"action": 4, "range": [current_pos, current_pos], "type": 2}
                    )
                    break
                actions.append(
                    {"action": 4, "range": [current_pos, current_pos], "type": 2}
            ...
            for current_pos in range(1, no_tokens):
                if not isinstance(
                    self.base_token.token_list[current_pos],
                    IdentifierRepresentation_Token,
                ):
                    actions.append(
                        {"action": 18, "range": [current_pos, current_pos], "type": 0}
                    )

        if (
            Token.Category.SYNTAX_FIXING in action_category
            or Token.Category.BEHAVIOR_CHANGING in action_category
        ):
            # add and token
            for current_pos in range(1, no_tokens + 1):
                if (
                    current_pos < no_tokens
                    and isinstance(self.base_token.token_list[current_pos], AND_Token)
                ) or (
                    current_pos - 1 >= 0
                    and isinstance(
                        self.base_token.token_list[current_pos - 1], AND_Token
                    )
                ):
                    pass
                elif current_pos < no_tokens and isinstance(
                    self.base_token.token_list[current_pos], FullComment_Token
                ):
                    actions.append(
                        {"action": 7, "range": [current_pos, current_pos], "type": 0}
                    )
                    break
                else:
                    actions.append(
            ...
            for current_pos in range(no_tokens):
                if (
                    ...
                    and self.base_token.token_list[current_pos].category()
                    == Token.Category.BEHAVIOR_CHANGING
                ):
                    actions.append(
                        {"action": 18, "range": [current_pos, current_pos], "type": 0}
                    )
        if (
            Token.Category.SANATISATION_ESCAPING in action_category
            or Token.Category.SYNTAX_FIXING in action_category
        ):
            # capitalise random pos
            for current_pos in range(no_flat_tokens):
                if (
                    isinstance(
                        self.base_token.get_token_flat_idx(current_pos),
                        KeywordRepresentation_Token,
                    )
                    and self.base_token.get_token_flat_idx(current_pos).is_all_caps()
                ):
                    actions.append(
                        {"action": 13, "range": [current_pos, current_pos], "type": 0}
                    )
            ...
            for current_pos in range(no_tokens):
                ...
                if isinstance(token, AND_Token) and token.whitespace_type_value() == 0:
                    actions.append(
                        {"action": 17, "range": [current_pos, current_pos], "type": 0}
                    )
            ...
            for current_pos in range(no_tokens):
                ...
                if isinstance(token, AND_Token) and token.type_value() == 0:
                    actions.append(
                        {"action": 19, "range": [current_pos, current_pos], "type": 0}
                    )
    ...
    def get_all_actions(self, max_timstamp):
        ...
        for current_timestamp in range(max_timstamp * 4):
            all_actions[str([1, current_timestamp, 0])] = [
                0 for x in range(max_timstamp)
            ]
            all_actions[str([3, current_timestamp, 1])] = [
                0 for x in range(max_timstamp)
            ]
            all_actions[str([4, current_timestamp, 0])] = [
                0 for x in range(max_timstamp)
            ]
            all_actions[str([4, current_timestamp, 1])] = [
                0 for x in range(max_timstamp)
            ]
            all_actions[str([4, current_timestamp, 2])] = [
                0 for x in range(max_timstamp)
            ]
            all_actions[str([18, current_timestamp, 0])] = [
                0 for x in range(max_timstamp)
            ]
            all_actions[str([7, current_timestamp, 0])] = [
                0 for x in range(max_timstamp)
            ]
            all_actions[str([13, current_timestamp, 0])] = [
                0 for x in range(max_timstamp)
            ]
            all_actions[str([17, current_timestamp, 0])] = [
                0 for x in range(max_timstamp)
            ]
            all_actions[str([19, current_timestamp, 0])] = [
                0 for x in range(max_timstamp)
            ]
            all_actions[str([20, current_timestamp, 0])] = [
                0 for x in range(max_timstamp)
            ]
