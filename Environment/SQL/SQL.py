...
from Environment.Tokens_Actions.Basic_Block.IdentifierRepresentation_Token import (
    IdentifierRepresentation_Token,
)

from Environment.Tokens_Actions.Basic_Block.OperatorRepresentation_Token import (
    OperatorRepresentation_Token,
)
from Environment.Tokens_Actions.Basic_Block.NumberRepresentation_Token import (
    NumberRepresentation_Token,
)
from Environment.Tokens_Actions.Basic_Block.Comma_Token import Comma_Token
from Environment.Tokens_Actions.Basic_Block.Comment_Token import (
    Comment_Category,
    Comment_Token,
)
from Environment.Tokens_Actions.Basic_Block.HexRepresentation_Token import (
    HexRepresentation_Token,
)
...
from Environment.Tokens_Actions.Basic_Block.StringRepresentation_Token import (
    StringRepresentation_Token,
)
...
class SQL:
    def __init__(self, sql_statment: str, identifier: str, tokens=None) -> None:
        ...
        if tokens == None:
            self.__tokens, self.sql_starting_stmt = SQL.parse_string(
                sql_statment, identifier
            )
        else:
            ...
            if (i == len(splits) or i == 0) and (current_match == ""):
                ...
                whitespace = (
                    r"|".join(
                        [
                            re.escape(e)
                            for e in OperatorRepresentation_Token.special_operators
                        ]
                    )
                    + r"|"
                    + r"|".join(
                        [
                            re.escape(e)
                            for e in Whitespace_Token.whitespace_type_mapping.values()
                        ]
                    )
                    + r"|"
                    + r"|".join(
                        [
                            re.escape(e)
                            for e in Comment_Token.comment_type_mapping.values()
                        ]
                    )
                    + r"|"
                    + r"|".join(
                        [re.escape(e) for e in Quote_Token.quote_type_mapping.values()]
                    )
                    + r"|"
                    + r"|".join(
                        [
                            re.escape(e)
                            for e in Paranthesis_Token.paranthesis_type_mapping.values()
                        ]
                    )
                )
    ...
    def convert_words_to_tokens(words: list, identifier: str):
        ...
        while changed:
            ...
            while current_index < len(last_pass):
                ...
                if (
                    isinstance(last_pass[current_index], OperatorRepresentation_Token)
                    and current_index + 1 < len(last_pass)
                    and isinstance(
                        last_pass[current_index + 1], OperatorRepresentation_Token
                    )
                    and OperatorRepresentation_Token.is_operator(
                        str(last_pass[current_index])
                        + str(last_pass[current_index + 1])
                    )
                ):
                    current_pass.append(
                        OperatorRepresentation_Token.parse(
                            str(last_pass[current_index])
                            + str(last_pass[current_index + 1])
                        )
                    )
                ...
                # full comement --
                    ...
                    current_index + 2 < len(last_pass)
                    and isinstance(
                        last_pass[current_index], OperatorRepresentation_Token
                    )
                    and isinstance(
                        last_pass[current_index + 1], OperatorRepresentation_Token
                    )
                    and isinstance(last_pass[current_index + 2], Whitespace_Token)
                    and Comment_Token.is_comment(
                        str(last_pass[current_index])
                        + str(last_pass[current_index + 1])
                        + str(last_pass[current_index + 2])
                    )
                ):
                    comment_Token = Comment_Token.parse(
                        str(last_pass[current_index])
                        + str(last_pass[current_index + 1])
                        + str(last_pass[current_index + 2])
                    )
                ...
                # # comment range
                    ...
                    open_comment_Token = Comment_Token.parse(
                        str(last_pass[current_index])
                        + str(last_pass[current_index + 1])
                    )
                ...
                # if keyword
                    ...
                    isinstance(last_pass[current_index], StringRepresentation_Token)
                    and KeywordRepresentation_Token.is_keyword(
                        str(last_pass[current_index])
                    )
                    and current_index < len(last_pass)
                    and not (
                        current_index > 1
                        and isinstance(last_pass[current_index - 1], Quote_Token)
                        and isinstance(last_pass[current_index + 1], Quote_Token)
                    )
                ):
                    current_pass.append(
                        KeywordRepresentation_Token.parse(str(last_pass[current_index]))
                    )
                    current_index += 1
                    changed = True
    ...
    def get_in_between_quotes(
        self, payload: Payload, str_1_idx: int, str_2_idx: int
    ) -> list:
        """
        ...
        for current_payload_token_idx in range(len(payload_tokens)):
            if (
                current_payload_token_idx == str_1_idx
                and current_payload_token_idx == str_2_idx
            ):
                regex.append(
                    re.escape(str(str(payload_tokens[current_payload_token_idx])))
                    + r"(.*?)"
                )
    ...
    def is_affective_behviour_changing(
        self,
        payload: Payload,
        payload_identifier_token_base_idx: int,
        payload_token_2_base_idx: int,
    ):
        """
        ...
        assert isinstance(
            payload_base_tokens[payload_identifier_token_base_idx],
            IdentifierRepresentation_Token,
        )
        ...
        if quote_open_before_ident > 0:
            ...
            quote_list = self.get_in_between_quotes(
                payload, payload_identifier_token_base_idx, payload_token_2_base_idx
            )
            is_outside_identifer_Context, lefted_quotes = SQL.list_after(
                quote_list, open_quotes[0]
            )
        ...
        while current_token < payload_token_2_base_idx:
            if isinstance(
                payload_base_tokens[payload_identifier_token_base_idx + current_token],
                FullComment_Token,
            ) or (
                isinstance(
                    payload_base_tokens[
                        payload_identifier_token_base_idx + current_token
                    ],
                    Comment_Token,
                )
                and payload_base_tokens[
                    payload_identifier_token_base_idx + current_token
                ].comment_category()
                == Comment_Category.Full_Comment
            ):
                is_commented = True
                break
            if (
                ...
                == Comment_Category.Open_Range_Comment
            ):
                ...
                == Comment_Category.Close_Range_Comment
            ):
                if len(pstack) > 0:
    ...
    def is_keyword_after_payload_commented(
        self, payload, identifer_token_base_index: int
    ):
        # payload flat tokens
        payload_base_tokens = payload.get_tokens_base_idx()
        ...
        for current_token_idx, current_token in enumerate(payload_base_tokens):
            if isinstance(
                current_token, FullComment_Token
            ) and self.is_affective_behviour_changing(
                payload, identifer_token_base_index, current_token_idx
            ):
                is_sql_keyowrds_after_payload_commented = (
                    self.is_sql_keywords_after_payload(payload)
                )
                break
    ...
    def is_sql_keywords_after_payload(self, payload: Payload):
        ...
        if results_1 is not None:
            ...
            if any(
                y in after_payload_str.casefold()
                for y in KeywordRepresentation_Token.reserved_keywords
            ):
                return True
            else:
                return False
    ...
    def contains(self, payload: Payload, flat_idx: int):
        ...
        if re.search(regex, self.sql_statment) is not None:
            return re.search(regex, self.sql_statment).groups(1)[0] == str(
                payload_tokens[flat_idx]
            )
        else:
            return False
    ...
    def is_sanatized(self, payload: Payload, paylaod_flat_token_idx: int):
        ...
        if match_found:
            ...
            if match_string.casefold().startswith(
                str(payload_flat_tokens[paylaod_flat_token_idx]).casefold()
            ):
                return None
            else:
    ...
    def replace_identifier_with_payload(self, payload: Payload):
        ...
        self.__tokens.replace_flat_idx_token_with_tokens(
            identifier_token_flat_index, paylod_flat_tokens
        )
        ...
        while current_token_index < len(flat_index_sql_tokens):
            if is_ident_reached:
                if len(closing_quotes) > 0:
                    if (
                        str(flat_index_sql_tokens[current_token_index])
                        == closing_quotes[0]
                    ):
                        closing_quotes.pop(0)
                    else:
                        in_context_elements.append(
                            flat_index_sql_tokens[current_token_index]
                        )
                else:
                    break
