...
class SQL_Proxy:
    ...
    def get_all_sql_statments(self, data_input):
        ...
        with open(self.file_name, "r", errors="ignore") as content_file:
            ...
            possible_sql = [
                (
                    re.search(r"\S*\s*\S*\s*\S*\s*(.*)", currentline).group(1),
                    currentline,
                )
                for currentline in contents_split
                if data_input.token in currentline
            ]
    ...
    def get_new_sql(self, sql_input: Input):
        with open(self.file_name, "r", errors="ignore") as content_file:
            ...
            possible_sql = [
                ...
                for currentline in contents_split
                if sql_input.token in currentline
            ]
            possible_sql = [x[0] for x in possible_sql]
            pp = PrettyPrinter(indent=4)
            ...
            if len(possible_sql) > 1:
                lengths = [
                    SQL_Proxy.LCSubStr(
                        sql_input.sql_starting_stmt,
                        current_statment,
                        len(sql_input.sql_starting_stmt),
                        len(current_statment),
                    )
                    for current_statment in possible_sql
                ]
                possible_sql = [
                    current_statment
                    for current_statment in possible_sql
                    if SQL_Proxy.LCSubStr(
                        sql_input.sql_starting_stmt,
                        current_statment,
                        len(sql_input.sql_starting_stmt),
                        len(current_statment),
    ...
    def fix_mysql_file_lines(lines: list):
        ...
        while index < len(lines):
            if (
                re.search(
                    "[0-9]+-[0-9]+-[0-9]+T[0-9]+:[0-9]+:[0-9]+.[0-9]+Z", lines[index]
                )
                or "mysqld, Version: 5.6.51 (MySQL Community Server (GPL)). started with:"
                in lines[index]
                or re.search("\x00", lines[index])
            ):
                index += 1
            else:
                if index - 1 < 0:
