...
class CommentRange_Token(Token):
    ...
    def __init__(self, commented_part_tokens, type) -> None:
        ...
        assert all(
            [
                isinstance(current_token, Token)
                for current_token in commented_part_tokens
            ]
        )
        assert (
            type in CommentRange_Token.comment_range_type_to_comment_type_mapping.keys()
        )
        self.commented_part_tokens = commented_part_tokens
        self.token_list = [
            Comment_Token(
                CommentRange_Token.comment_range_type_to_comment_type_mapping[type][0]
            )
        ]
        ...
        self.token_list.extend(
            [
                Comment_Token(
                    CommentRange_Token.comment_range_type_to_comment_type_mapping[type][
                        1
                    ]
                )
            ]
