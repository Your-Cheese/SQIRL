...
class Paranthesis_Token(Token):
    ...
    def is_paranthesis(s: str):
        return (
            True if s in Paranthesis_Token.paranthesis_type_mapping.values() else False
        )

    def parse(s: str):
        assert Paranthesis_Token.is_paranthesis(s)
        type = list(Paranthesis_Token.paranthesis_type_mapping.keys())[
            list(Paranthesis_Token.paranthesis_type_mapping.values()).index(s)
        ]
