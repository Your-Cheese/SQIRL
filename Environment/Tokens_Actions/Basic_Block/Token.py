...
class Token:
    ...
    def replace_flat_idx(self, pos: int, token, current_global_pos=-1):
        ...
        for current_token in self.token_list:
            ...
            if is_basic_token:
                ...
                is_finished, current_global_pos = current_token.replace_flat_idx(
                    pos, token, current_global_pos
                )
                if is_finished:
                    return True, current_global_pos
    ...
    def replace_flat_idx_token_with_tokens(
        self, pos: int, tokens: list, current_global_pos=-1
    ):
        assert isinstance(pos, int)
        local_idx = -1
        for current_token in self.token_list:
    ...
    def remove_token_flat_idx(self, pos: int, current_global_pos=-1):
        ...
        for current_token in self.token_list:
            ...
            if is_basic_token:
                ...
                is_finished, current_global_pos = current_token.remove_token_flat_idx(
                    pos, current_global_pos
                )
                if is_finished:
                    return True, current_global_pos
        return False, current_global_pos
