...
class DQNWorker:
    ...
    def __init__(
        ...
        self.Q_value, self.load_mem, self.save_dir_mem = DQNWorker.load_model(
            server_host, server_port, state_size, action_size, domain
        )
        ...
        self.optimizer = torch.optim.Adam(
            self.Q_value.parameters(), lr=self.learning_rate
        )
    ...
    def load_model(server_host, server_port, state_size, action_size, domain):
        ...
        if ack != "ACK":
            raise Exception(
                f"[DQN_Worker-> sync_Q_target] got responce {ack}, should be ACK"
            )
        ...
        load_path = (
            init_parameter["load_loc"] + "/Q_value_id.mem"
            if init_parameter["load_loc"] is not None
            else ""
        )
        save_dir_mem = (
            init_parameter["save_loc"].split("Q_value.model")[0] + "Q_value_id.mem"
        )
