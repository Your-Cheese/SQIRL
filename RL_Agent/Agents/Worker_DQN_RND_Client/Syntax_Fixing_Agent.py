...
class Syntax_Fixing_Agent:
    def __init__(self, agent_id, domain, learning=True) -> None:
        ...
        server_port = 1234
        self.action_Q_value = DQNWorker(
            State_Representation.size() + 5,
            1,
            f"syntax_fixing_action_{agent_id}",
            server_host,
            server_port,
            domain,
            learning,
        ...
        self.rdn = RDN(
            State_Representation.size() + 5, 1000, f"syntax_fixing_action_{agent_id}"
        )
        self.epsilon = 0.6
    ...
    def get_next_action(self, current_state, representation_vector, explore):
        ...
        if explore and eps_random < self.epsilon:
            ...
            best_action, best_state, best_q_value = self.get_best_action(
                actions, representation_vector
            )
            selected_action = {
        ...
        if self.last_state != None and self.last_selected_action != None:
            ...
            if current_state["game"] == Game_Type.DONE:
                ...
                actions = [
                    ...
                    for current_action in current_state["actions"]
                ]
                best_action, best_state, current_q_value = self.get_best_action(
                    actions, representation_vector
                )
            ...
            results_file = open(
                self.action_Q_value.save_dir_mem.split("Checkpoint")[0]
                + f"rewards_RDN{self.agent_id}.stats",
                "a",
            )
            ...
            # cache them
            self.action_Q_value.cache(
                last_q_value, current_q_value.item(), reward + interensic_reward
            )
