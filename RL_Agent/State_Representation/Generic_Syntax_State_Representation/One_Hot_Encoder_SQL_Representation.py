...
class One_Hot_Encoder_SQL_Representation:
    ...
    def embedding(sql: SQL):
        ...
        for current_token in sql.get_tokens().flat_idx_tokens_list():
            result.append(
                One_Hot_Encoder_SQL_Representation.token_to_idx[
    ...
    def is_valid_embedding(embed):
        return (
            True
            if embed >= 0
            and embed < len(One_Hot_Encoder_SQL_Representation.tokens_embedding)
            else False
        )
    ...
    def generate_representation(self, sql: SQL):
        ...
        one_hot_encoded_data = [
            One_Hot_Encoder_SQL_Representation.one_hot_encode(current_embeding)
