...
class SQL_Generic_Parser:
    ...
    def convert_generic(self, sql_stmt, token):
        ...
        for current_token in result.tokens:
            if (
                current_token.is_name
                or current_token.is_float
                or current_token.is_integer
                or current_token.is_integer
                or current_token.is_wildcard
                or (
                    current_token.value.startswith("'")
                    and current_token.value.endswith("'")
                )
            ):
    ...
    def convert_syntax_generic(self, sql_stmt, token):
        ...
        for current_token in result.tokens:
            if token in current_token.value:
                ...
                current_token.is_punctuation
                or current_token.is_left_parenthesis
                or current_token.is_right_parenthesis
            ):
    ...
    def check_keyword(self, w, result):
        if w.span()[0] - 1 < 0 and w.span()[0] - 1 > len(w.group()):
            ...
            return (
                w.group().casefold() in self.reserved_keywords
