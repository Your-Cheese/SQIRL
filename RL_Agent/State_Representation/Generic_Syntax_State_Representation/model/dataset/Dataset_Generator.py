...
from Environment.Tokens_Actions.Sanitization_Escaping.WhitespaceConverter_Action import (
    WhitespaceConverter_Action,
)
from RL_Agent.State_Representation.Generic_Syntax_State_Representation.SQL_Representation import (
    SQL_Representation,
)
from RL_Agent.State_Representation.Generic_Syntax_State_Representation.model.dataset.SQL_Generator import (
    SQL_Generator,
)
import random
import os
...
class Dataset_Generator:
    ...
    def generate_syntax_dataset(self, size):
        ...
        generic_stmt_path = os.path.join(
            self.dataset_file, "generic_synatx_statment.dataset"
        )
    ...
    def replace_whitespace_with_no_white_space(self, sql: SQL):
        ...
        if random.randint(0, 1):
            ...
            for current_pos in range(flat_idx_length):
                if isinstance(
                    sql.get_tokens().get_token_flat_idx(current_pos), Whitespace_Token
                ):
                    possible_locations.append(current_pos)
            # pick random pos
            pos = random.choice(possible_locations)
    ...
    def captilize_keywords(self, sql: SQL):
        ...
        if random.randint(0, 1):
            ...
            for current_pos in range(flat_idx_length):
                if isinstance(
                    sql.get_tokens().get_token_flat_idx(current_pos),
                    KeywordRepresentation_Token,
                ):
                    possible_locations.append(current_pos)
            # pick random pos
    ...
    def convert_SQL(self, statment):
        ...
        for current_literal in literals:
            result_literals.extend(
                Dataset_Generator.find_parens(statment, current_literal)
            )
        token = self.tokenizor.get_next_token()
        pos_token = random.choice(result_literals)
        assert len(current_literal) > 0
        statment = (
            "".join(statment[: pos_token[0] + 1])
            + str(token)
            + "".join(statment[pos_token[1] :])
        )
