...
class MYSQL_Generator:
    ...
    def __init__(self) -> None:
        ...
        self.words = MYSQL_Generator.remove_values_from_list(
            self.words, self.reserved_keywords
        )
    ...
    def select(self, depth=0, column_size=None, query=None):
        ...
        column_fields = [
            Field(current_column_name) for current_column_name in columns_names
        ]
        ...
        if random.randint(0, 1):
            ...
            if random.randint(0, 1):
                # alias
                column_fields[chosen_column] = fn.Concat(
                    column_fields[chosen_column], " ", Field(random.choice(self.words))
                ).as_(random.choice(self.words))
            else:
                # no alias
                column_fields[chosen_column] = fn.Concat(
                    column_fields[chosen_column], " ", Field(random.choice(self.words))
        ...
        if random.randint(0, 1):
            ...
            for current_case in range(number_of_cases):
                chosen_column = random.randint(0, no_columns - 1)
                case = case.when(
                    self.critertion_parameter(
                        1,
                        [
                            Field(random.choice(self.words)),
                            Field(random.choice(self.words)),
                        ],
                    ),
                    str(random.choice(self.words) + " " + random.choice(self.words)),
                )
        ...
        if random.randint(0, 1):
            ...
            if column_size is not None:
                no_columns = (
                    random.randint(1, (column_size // self.select_column_adders) // 2)
                    * 2
                )
            else:
            ...
            for current_operation in range(no_operations):
                ...
                current_operation_columns = [
                    random.choice(columns_fields),
                    random.choice(columns_fields),
                ]
                if random_oper == 1:
                    # + operation
                    operations.append(
                        current_operation_columns[0] + current_operation_columns[1]
                    )
                elif random_oper == 2:
                    # - operation
                    operations.append(
        ...
        if random.randint(0, 1):
            ...
            for current_aggregated_column in range(no_columns):
                ...
                if random_oper == 1:
                    # abs
                    result_aggerated_columns.append(
                        fn.Abs(columns_fields[current_aggregated_column])
                    )
                elif random_oper == 2:
                    # avg
                    result_aggerated_columns.append(
                        fn.Avg(columns_fields[current_aggregated_column])
                    )
                elif random_oper == 3:
                    # count
                    result_aggerated_columns.append(
                        fn.Count(columns_fields[current_aggregated_column])
                    )
                elif random_oper == 4:
                    # floor
                    result_aggerated_columns.append(
                        fn.Floor(columns_fields[current_aggregated_column])
                    )
                elif random_oper == 5:
                    # max
                    result_aggerated_columns.append(
                        fn.Max(columns_fields[current_aggregated_column])
                    )
                elif random_oper == 6:
                    # Bin
                    result_aggerated_columns.append(
                        fn.Bin(columns_fields[current_aggregated_column])
                    )
                elif random_oper == 7:
                    # min
                    result_aggerated_columns.append(
                        fn.Min(columns_fields[current_aggregated_column])
                    )
                elif random_oper == 8:
                    # std
                    result_aggerated_columns.append(
                        fn.Std(columns_fields[current_aggregated_column])
                    )
                else:
                    # stdDev
                    result_aggerated_columns.append(
                        fn.StdDev(columns_fields[current_aggregated_column])
                    )
            ...
            if random.randint(0, 1):
                ...
                columns_names = random.choices(self.words, k=no_columns)
                columns_fields = [
                    Field(current_column) for current_column in columns_names
                ]
        ...
        if (
            ...
            and no_sub_queries < self.max_sub_query
            and (
                (column_size is not None and no_columns_added < column_size)
                or column_size is None
            )
        ):
            no_columns_added += 1
        ...
        if (
            random.randint(0, 1)
            and no_columns_added % 2 == 0
            and no_columns_added > 8
            and depth < self.max_depth
        ):
    ...
    def insert(self, depth=0):
        ...
        if random.randint(0, 1):
            ...
            no_columns = random.randint(1, self.column_limit)
            columns_names = [
                Field(random.choice(self.words)) for c in range(no_columns)
            ]
    ...
    def critertion_parameter(self, no_filters, columns_fields):
        ...
        for current_filter in range(no_filters):
            ...
            current_filter_columns_1 = [
                random.choice(columns_fields),
                random.choice(columns_fields),
                self.date_operands(False),
                self.date_operands(False),
            ]
            current_filter_columns_2 = [
                random.choice(columns_fields),
                random.choice(columns_fields),
                self.date_operands(True),
                self.date_operands(True),
            ]
            ...
            if random_number_of_operands == 2:
                ...
                # three operands
                operand_1 = self.arithmatic_operation(
                    random.choice(current_filter_columns_1),
                    random.choice(current_filter_columns_2),
                )
                operand_2 = random.choice(current_filter_columns_2)
            ...
            if random_oper == 1:
                ...
                if random.randint(0, 1):
                    # with tuple
                    operation = Tuple(
                        current_filter_columns_1[0], current_filter_columns_1[1]
                    ).isin([Tuple(oper_columns_choices[0], oper_columns_choices[1])])
                else:
                    # without tuple
                    operation = current_filter_columns_1[0].isin(
                        [oper_columns_choices[0], oper_columns_choices[1]]
                    )
            elif random_oper == 8:
                # BETWEEN operation
                ...
                numbers_choosen = []
                if random.randint(0, 1):
                    numbers_choosen.append(
                        round(random.random() * random.randint(1, 100), 4)
                    )
                    numbers_choosen.append(
                        round(random.random() * random.randint(1, 100), 4)
                    )
                ...
                operation = current_filter_columns_1[0][
                    numbers_choosen[0] : numbers_choosen[1]
                ]
            elif random_oper == 9:
                # like operation
