...
class One_Hot_Encoder_Payload_Representation:
    ...
    def payload_embedding(payload: Payload):
        ...
        for current_token in tokens:
            result.append(
                One_Hot_Encoder_Payload_Representation.token_to_idx[
                    str(current_token).casefold()
                ]
            )
    ...
    def one_hot_encode(current_embeding):
        ...
        assert One_Hot_Encoder_Payload_Representation.is_valid_embedding(
            current_embeding
        )
        # convert to one hot encode
        return [
            1 if current_hot_encode == current_embeding else 0
            for current_hot_encode in range(
    ...
    def is_valid_embedding(embed):
        return (
            True
            if embed >= 0
            and embed < len(One_Hot_Encoder_Payload_Representation.tokens_embedding)
            else False
        )
    ...
    def generate_representation(self, payload: Payload):
        ...
        # embed data
        embedded_data = One_Hot_Encoder_Payload_Representation.payload_embedding(
            generic_sql
        )
        ...
        one_hot_encoded_data = [
            One_Hot_Encoder_Payload_Representation.one_hot_encode(current_embeding)
            for current_embeding in embedded_data
        ]
