...
save_file_model = os.path.join(
    "stats_logs", "domain_time", "Checkpoint_Worker_Server", "Q_value.model"
)
...
def service_client(connection):
    ...
    if "INIT" in message:
        ...
        init_parameters = pickle.dumps(
            {
                "save_loc": save_file_model,
                "load_loc": load_path,
    ...
    # else if update parameters
        ...
        # else wait for all clients and update
            ...
            print(
                f"waiting for other clients before update current clients arrived: {get_barrier_value()}"
            )
    ...
    for current_key in agents_paramters[0]:
        for current_model_index in range(len(agents_paramters)):
            if current_model_index == 0:
                stored_data[current_key] = agents_paramters[current_model_index][
                    current_key
                ]
            else:
                stored_data[current_key] = (
                    stored_data[current_key]
                    + agents_paramters[current_model_index][current_key]
                )
...
def accept_connections(ServerSocket):
    ...
    print(
        "[Worker Server->accept connections] Connected to: "
        + address[0]
        + ":"
        + str(address[1])
    )
