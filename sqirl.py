...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        ...
        parser.add_option(
            ...
            action="store",
            dest="num_agents",
            help="Number of agents to run with SQIRL is 1 unless Worker structure invoked",
            default=1,
        )
        ...
        if agent_type == 4 or (
            ...
            try:
                ...
                if model_dir is not None:
                    serve_sub_pro = subprocess.Popen(
                        [
                            "python3",
                            "Worker_DQN_RND_Server.py",
                            "-u",
                            num_agents,
                            "--model_dir",
                            model_dir,
                    ...
                    server_sub_pro = subprocess.Popen(
                        ["python3", "Worker_DQN_RND_Server.py", "-u", num_agents]
                    )
                    time.sleep(2)
                sub_pros = []
                for agent_id in range(1, int(num_agents) + 1):
                    ...
                    if options.login_function is not None:
                        call.extend(
                            [
                                "--login_function_name",
                                str(options.login_function),
                                "--auth_file_path",
                                str(options.module_path),
                            ]
                        )
                    if model_dir is not None:
            ...
            if int(num_agents) > 1:
                print("Setting --num_agents to 1")
                num_agents = 1
            log_output = sqril_cmd(
                print,
                target_url,
                max_depth,
                verbose,
