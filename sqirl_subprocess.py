...
def main():
    ...
    if len(sys.argv) < 2:
        ...
        parser.add_option(
            ...
            "--agent_unique_id",
            action="store",
            dest="agent_unique_id",
            help="ID of the agent used for logging",
            default=1,
        )
        ...
        parser.add_option(
            ...
            action="store",
            dest="level",
            help="Depth for the crawler to traverse",
            default="0",
        )
        ...
        parser.add_option(
            ...
            action="store",
            dest="db_type",
            help="Type of Database e.g. mysql",
            default="mysql",
        )
        ...
        parser.add_option(
            ...
            action="store",
            dest="log_file",
            help="Path to the log file of the SQL database",
            default=None,
        )
        ...
        parser.add_option(
            ...
            action="store",
            dest="episodes",
            help="Maximum number of episodes per input found",
            default=10,
        )
        ...
        parser.add_option(
            ...
            action="store",
            dest="win_criteria",
            help="Minimum number of vulnerabilities found before switching inputs",
            default=1,
        )
        ...
        parser.add_option(
            ...
            action="store",
            dest="loss_criteria",
            help="Maximum number of episodes before switching inputs",
            default=10,
        )
        ...
        parser.add_option(
            ...
            help="Method to select next input: 1 FIFO queue, 2 is random",
            default=2,
        )
        ...
        parser.add_option(
            ...
            action="store",
            dest="model_dir",
            help="Directory containing model checkpoints, if not set a new agent will begin training",
            default=None,
        )
        ...
        parser.add_option(
            ...
            action="store",
            dest="train",
            help="Boolean to set if SQIRL is in learning mode",
            default=False,
        )
        ...
        if (
            options.login_function != None
            and options.module_path == None
            or options.login_function == None
            and options.module_path != None
        ):
            raise Exception(
                "If using authentication both --login_function_name and --auth_file_path must be set."
            )
        else:
            login_module = {
                "module_path": options.module_path,
                "function": options.login_function,
            }

        if (
            options.model_dir != None
            and options.agent_type != None
            or options.model_dir == None
            and options.agent_type == None
        ):
            raise Exception(
                "Please set only one of --model_dir (if you want to load a previous model), or '--agent if you wish to train a new agent"
            )
        ...
        if int(agent_type) != 4:
            ...
            os.rename(
                "./stats_logs/all_inputs_found.stat",
                f"./{log_location}/all_inputs_found.stat",
            )

        # create instance of agent based on supplied agent type
        if agent_type == 0:  # Random
            agent = Agent_Random(agent_unique_id)
        elif agent_type == 1 or (
            model_dir is not None and "DQN_Agent" in model_dir
        ):  # DQN
        ...
        elif agent_type == 2 or (
            model_dir is not None and "DQN_RND_Agent" in model_dir
        ):  # DQN_RND
            if model_dir is None:
                model_dir = os.path.join(
                    "/RL_Agent", "pretrained_agents", "DQN_RND_Agent"
                )
                model_dir = os.path.abspath(os.getcwd() + model_dir)
            model_checkpoint_file = os.path.join(
                os.getcwd(), log_location, "DQN_RND_Agent_Checkpoint"
            )
            model_checkpoint_file = os.path.abspath(model_checkpoint_file)
            agent = Agent_6(
                agent_unique_id,
        ...
        elif agent_type == 3 or (
            model_dir is not None and "One_Hot_Encoder_DQN_Agent" in model_dir
        ):  # One_Hot_Encoder_DQN_RND
            if model_dir is None:
                model_dir = os.path.join(
                    "/RL_Agent", "pretrained_agents", "One_Hot_Encoder_DQN_Agent"
                )
                model_dir = os.path.abspath(os.getcwd() + model_dir)
            model_checkpoint_file = os.path.join(
                os.getcwd(), log_location, "One_Hot_Encoder_DQN_Agent_Checkpoint"
            )
            model_checkpoint_file = os.path.abspath(model_checkpoint_file)
            agent = Agent_8(
                agent_unique_id,
        ...
        elif agent_type == 4 or (
            model_dir is not None and "Worker_DQN_RND_Client " in model_dir
        ):  # Worker_DQN_RND
            agent = Agent_11(agent_unique_id, learning=is_learning, domain=domain)
            log_location = agent.syntax_fixing_agent.action_Q_value.save_dir_mem.split(
                "/Checkpoint"
            )[0]
            if int(agent_unique_id) == 1:
                os.rename(
        ...
        if int(agent_unique_id) == 1:
            print(
                f"Log output location: {os.path.abspath(os.path.join(os.getcwd(), log_location))}"
            )
        ...
        results_file = open(
            os.path.join(log_location, f"result_stats_{agent_unique_id}.stats"), "w+"
        )
        results_file.close()

        results_file = open(
            os.path.join(log_location, f"log_{agent_unique_id}.stats"), "w+"
        )
        results_file.close()
        ...
        training_loss = {
            Game_Type.BEHAVIOR_CHANGING: {"action": [], "range": [], "type": []},
            Game_Type.SANITIZATION_ESCAPING: {"action": [], "range": [], "type": []},
            Game_Type.SYNTAX_FIXING: {"action": [], "range": [], "type": []},
        }

        results_file = open(
            os.path.join(log_location, f"result_stats_{agent_unique_id}.stats"), "a"
        )
        results_file.write("---------------------Training-----------------\n")
        results_file.close()

        results_file = open(
            os.path.join(log_location, f"timing_{agent_unique_id}.stats"), "a+"
        )
        results_file.close()

        results_file = open(
            os.path.join(log_location, f"actions_Applied_{agent_unique_id}.stats"), "w+"
        )
        results_file.close()

        sql_statment_file = open(
            os.path.join(log_location, f"sql_statment_{agent_unique_id}.stats"), "w+"
        )
        sql_statment_file.close()

        results_file = open(
            os.path.join(log_location, f"rewards_{agent_unique_id}.stats"), "w+"
        )
        results_file.close()
        avg_time_taken = 0
        ...
        for current_episode in range(no_episodes):
            sql_statment_file = open(
                os.path.join(log_location, f"sql_statment_{agent_unique_id}.stats"), "a"
            )
            sql_statment_file.write(
            ...
            current_game, current_state = env.reset(
                change_input=reached_win_criteria or reached_loss_criteria
            )

            stat_log_file = open(
                os.path.join(log_location, f"log_{agent_unique_id}.stats"), "a"
            )
            stat_log_file.write(
                f"\n\n---------------------ep{current_episode}-----------------\n"
            )
            ...
            results_file = open(
                os.path.join(log_location, f"actions_Applied_{agent_unique_id}.stats"),
                "a",
            )
            results_file.write(
                f"---------------------ep{current_episode}-----------------\n"
            ...
            if current_episode == 0:
                results_file = open(
                    os.path.join(log_location, f"result_stats_{agent_unique_id}.stats"),
                    "a",
                )
                results_file.write(
                ...
                results_file.close()
                seen_pairs[env.current_input_entry.input.action] = [
                    param["name"]
                    for param in env.current_input_entry.input.inputs
                    if param["input_use"] == "injection_point"
                ]
                ...
                if env.current_input_entry.input.action in seen_pairs:
                    if [
                        ...
                        for param in env.current_input_entry.input.inputs
                        if param["input_use"] == "injection_point"
                    ][0] in seen_pairs[env.current_input_entry.input.action]:
                        continue
            ...
            if current_state["error"]:
                sql_statment_file.write("Base: None\n")
            else:
            ...
            while not game_done and not max_timestamp_reached:
                ...
                current_game, current_state, reward, game_done = env.step(
                    selected_action
                )
                results_file = open(
                    os.path.join(
                        log_location, f"actions_Applied_{agent_unique_id}.stats"
                    ),
                    "a",
                )
                results_file.write(
                    f'{current_time_stamp}: {current_state["payload"]}\n'
                )
                results_file.write("\n")
                results_file.close()
                ...
                stat_log_file.write(
                    f"{current_time_stamp}:Payload: {current_state['payload']}\n"
                )
                stat_log_file.write(
                    f"{current_time_stamp}:Available actions: {current_state['actions']}\n"
                )
                stat_log_file.write(
                    f"{current_time_stamp}:CURRENT GAME: {current_state['game']}\n"
                )
                stat_log_file.close()

                sql_statment_file = open(
                    os.path.join(log_location, f"sql_statment_{agent_unique_id}.stats"),
                    "a",
                )
                if current_state["error"]:
                    ...
                    sql_statment_file.write(
                        f"{current_time_stamp}: {current_state['sql']}\n"
                    )
                sql_statment_file.write("\n")
                sql_statment_file.close()
                ...
                if current_state["error"]:
                    stat_log_file.write("SQL EXECUTED: None\n")
                else:
            ...
            results_file = open(
                os.path.join(log_location, f"rewards_{agent_unique_id}.stats"), "a"
            )
            ...
            if current_wins >= win_criteria:
                ...
                results_file.write("[!] reached win criteria\n")
                ...
                stat_log_file.write("[!] WIN CRITERA\n")
                stat_log_file.close()
            ...
            if current_losses + current_wins >= loss_criteria:
                ...
                results_file.write("[!] reached loss criteria\n\n")
                results_file.close()
                stat_log_file = open(
                    os.path.join(log_location, f"log_{agent_unique_id}.stats"), "a"
                )
                stat_log_file.write("[!] LOSS CRITERA\n")
                stat_log_file.close()
